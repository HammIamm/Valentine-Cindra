...
async function typeText() {
    typingText.innerHTML = ""; // Kosongin dulu biar nggak ada teks aneh sebelumnya
    for (let i = 0; i < text.length; i++) {
        let paragraph = document.createElement("p"); // Bikin elemen <p> buat setiap paragraf
        typingText.appendChild(paragraph);
        
        for (let j = 0; j < text[i].length; j++) {
            paragraph.innerHTML += text[i][j]; // Tambahin teks satu per satu
            await new Promise(resolve => setTimeout(resolve, 25));
        }
        
        await new Promise(resolve => setTimeout(resolve, 500)); // Jeda sebelum lanjut ke paragraf berikutnya
    }
}

document.addEventListener("DOMContentLoaded", typeText); // Pastikan skrip jalan setelah halaman dimuat
